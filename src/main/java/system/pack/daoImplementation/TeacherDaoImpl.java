...
public class TeacherDaoImpl implements TeacherDaoInterface {
	...
	public Optional<TeacherEntity> findByName(String name) {
		...
		TypedQuery<TeacherEntity> query = entityManager.createQuery("select t from TeacherEntity t where t.name =:name",
				TeacherEntity.class);
	...
	public TeacherEntity findByLastName(String lastName) {
		...
		TypedQuery<TeacherEntity> query = entityManager
				.createQuery("select t from TeacherEntity t where t.lastName =:lastName", TeacherEntity.class);
	...
	public TeacherEntity findByIdentificationType(String identificationType) {
		...
		TypedQuery<TeacherEntity> query = entityManager.createQuery(
				"select t from TeacherEntity t where t.identificationType =:identificationType", TeacherEntity.class);
	...
	public TeacherEntity findByTeacherStatusId(int teacherStatusId) {
		...
		TypedQuery<TeacherEntity> query = entityManager.createQuery(
				"select t from TeacherEntity t where t.teacherStatus.teacherStatusId =:teacherStatusId",
				TeacherEntity.class);
	...
	public List<TeacherEntity> getTeachers(TeacherBean teacherBean) {
		...
		if (!teacherBean.getSubjectSearch().equals("")) {
			sql += ", subjects_by_teacher where " + "teachers.teacherId = subjects_by_teacher.teacherId"
		...
		if (!teacherBean.getSubjectSearch().equals("")) {
			...
			query.setParameter("subjectId", teacherBean.getSubjectSearch());
